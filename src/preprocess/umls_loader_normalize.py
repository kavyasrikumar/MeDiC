...
from MM_to_processed import load_medmentions_pubtator
...
def normalize_docs_and_save(docs, synonym_groups, manual_map, outdir):
    ...
    """
    docs_outdir = os.path.join(outdir, "MM_docs")
...
def main():
    ...
    # MRCONSO extraction & parse
    mrconso_path = ensure_mrconso_unzipped("MRCONSO.RRF.zip", "umls") if "MRCONSO.RRF.zip" else None
    cui_to_synonyms, cui_to_preferred = ({}, {}) if not mrconso_path else parse_mrconso(mrconso_path, source_vocab_filter=None)

    # load manual annotations (optional)
    ...
    # normalize docs and save
    os.makedirs("data/processed", exist_ok=True)
    normalize_docs_and_save(docs, synonym_groups, manual_map, "data/processed")
    save_synonym_groups_file(synonym_groups, os.path.join("data", "synonym_groups.json"))
